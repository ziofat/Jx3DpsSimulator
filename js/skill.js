	...
	this.createNew = function(skillData){
		...
		skill.calc = function(self,target,buffController,recipes,options){
			...
			if(this.damageInstant){
				...
				s = skill.name+" " + (flag.miss>0?"偏离":"")+(flag.insight>0?"识破":"")+(flag.crit>0?"会心":"")+(flag.hit>0?"命中":"") +" "+ damage;
				Utils.logln(s);
				Utils.calcDamage(damage);
				return damage;
			}else{
				s = skill.name+" " + (flag.miss>0?"偏离":"")+(flag.insight>0?"识破":"")+(flag.crit>0?"会心":"")+(flag.hit>0?"命中":"");
				Utils.logln(s);
				return 0;
			}
		};
